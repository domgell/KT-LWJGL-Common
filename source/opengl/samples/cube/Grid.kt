...
// TEMP
data class GridMeshData(val indices: IntArray, val vertices: FloatArray)
fun genGrid(): GridMeshData {
    fun addLine(vertices: ArrayList<Vector3fc>, indices: ArrayList<Int>, start: Vector3fc, end: Vector3fc) {
        val startIndex = vertices.size
        vertices.add(start)
        vertices.add(end)
    ...
    return GridMeshData(indices.toIntArray(), positions.toTypedArray().toFloatArray())
}